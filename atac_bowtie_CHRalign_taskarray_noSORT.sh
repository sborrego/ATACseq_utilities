...
#$ -o /som/sborrego/201810_ATACSEQ_MB468_R8/qsub_reports/bowtie_chr_alignment_noSORT_clip.out
#$ -e /som/sborrego/201810_ATACSEQ_MB468_R8/qsub_reports/bowtie_chr_alignment_noSORT_clip.err
...
ALIGN_CHR_DIR=${ALIGN_DIR}/chr_only_noSORT_alignments

mkdir -p ${ALIGN_CHR_DIR}

# Logs to keep track of things
RUNLOG=${ALIGN_DIR}/runlog_chrOnly_align_noSORT_${SGE_TASK_ID}.txt
echo "Run by `whoami` on `date`" > ${RUNLOG}

FLAG=${ALIGN_DIR}/alignment_chrOnly_errors_noSORT_${SGE_TASK_ID}.flagstat
...
BAM_FILE=${ALIGN_CHR_DIR}/${PREFIX}.bam

# Message for file alignment
echo "Chromosome Only noSORT Alignment Summary for ${PREFIX}" >> ${RUNLOG}
